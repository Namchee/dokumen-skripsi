...
Berdasarkan pembahasan mengenai syarat-syarat pembentukan pergerakan kolektif pada subbab \ref{sec:collective-movement}, terdapat 3 aspek penting yang harus dipertimbangkan dalam membuat sebuah definisi pergerakan kolektif:
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Inisialisasi sebuah himpunan $\mathcal{S}$ untuk setiap entitas $a$ yang terdapat pada $\mathcal{X}$. Pada awalnya, himpunan $\mathcal{S}$ hanya akan beranggotakan entitas $a$.
        ...
        \item Tambahkan himpunan $\mathcal{S}$ pada himpunan $\mathcal{T}$ apabila jumlah anggota himpunan $\mathcal{S}$ lebih besar atau sama dengan $m$.
        \item Tambahkan himpunan $\mathcal{T}$ pada himpunan $\mathcal{R}$.
    \end{enumerate}
    \item Kembalikan himpunan $\mathcal{R}$ sebagai hasil. Pada tahap ini, himpunan $\mathcal{R}$ akan berisi kumpulan himpunan rombongan yang terbentuk pada setiap interval waktu $I$ yang berdurasi selama $k$ satuan waktu.
\end{enumerate}
...
di mana lintasan dari sebuah entitas merupakan himpunan posisi yang terurut berdasarkan waktu secara menaik dalam ruang $\mathbb{R}^2$. Posisi yang tercatat dimulai dari $t = 0$. Beserta dengan himpunan entitas $\mathcal{X}$, diberikan pula $m = 2$, $k = 2$, $r = 3.5$, dan $\vartheta = 0.5$. Langkah-langkah yang dilakukan untuk melakukan identifikasi rombongan antara lain:
...
\begin{enumerate}
    ...
    \item Mengidentifikasi seluruh interval waktu konsekutif sepanjang $k$ yang mungkin dibuat. Berdasarkan nilai $k = 2$, interval waktu yang mungkin adalah $[0, 1]$ dan $[1, 2]$.
...
Berdasarkan langkah-langkah yang diambil, dapat diidentifikasi sebuah rombongan pada himpunan entitas $\mathcal{X}$ bernama $\mathcal{A}$ pada interval waktu $[1, 2]$ yang beranggotakan entitas $a$ dan $c$.
