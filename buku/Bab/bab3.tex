...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Untuk setiap himpunan entitas pada $\mathcal{S}$, hitung jarak \textit{dynamic time warping} dan \textit{cosine similarity} dari setiap anggota dari himpunan entitas $c$ dengan entitas lain yang 
        tidak termasuk dalam anggota $c$. Apabila jarak \textit{dynamic time warping} dari entitas $d$ dan setiap anggota dari himpunan $c$ lebih kecil atau sama dengan $r$ dan nilai \textit{cosine similarity} dari entitas $d$ dan setiap anggota dari himpunan $c$ lebih besar atau sama dengan $\vartheta$, tambahkan sebuah himpunan entitas baru $\mathcal{U}$ pada $\mathcal{S}$ di mana $\mathcal{U}$ merupakan salinan dari $c$ ditambah dengan entitas $d$.
...
\begin{algorithm}[t]
    ...
    \Fn{\Rombongan{$\mathcal{X}, m, k, r, \vartheta$}}{
        $rombongan \gets sebuah\;pemetaan\;anggota\;rombongan\;pada\;interval\;waktu$ \\
        ...
        \For{$(start, end)$  intervals}{
            ...
            \For{each entity $a$ in $\mathcal{X}$}{
                $entitySet \gets kumpulan\;kelompok\;pergerakan\;yang\;beranggotakan\;a$ \\
                \For{each entity $b$ in $\mathcal{X}$}{
                    \For{each entity group $c$ in $entitySet$}{
                        $isSimilarToAll \gets TRUE$ \\
                        
                        \For{each entity $d$ in $c$}{
                            $dtwC \gets DTWDistance(b, d)$ \\
                            $cosineC \gets cosineSimilarity(b, d)$ \\
                    
                            \If{$c \neq b$ AND $(dtwC > r$ OR $cosineC < \vartheta)$}{
                                $isSimilarToAll \gets FALSE$
                            }
                        }
                        
                        \If{$isSimilarToAll = TRUE$}{
                            $groupCpy \gets duplikat\;dari\;c$ \\
                            $groupCpy.insert(b)$ \\
                            $entitySet.insert(groupCpy)$
                        }
                    }
                    
                    $dtwA \gets DTWDistance(a, b)$ \\
                    $cosineA \gets cosineSimilarity(a, b)$ \\
                    
                    \If{$dtwA \leq r$ AND $cosineA \geq \vartheta$}{
                        $newGroup \gets [a, b]$ \\
                        $entitySet.insert(newGroup)$
                    }
                }
                
                \For{each entity group $\mathcal{G}$ in entitySet}{
                    \If{$\mathcal{G}.size() \geq m$}{
                        $collectiveMovements.insert(\mathcal{G})$
                    }
                }
            }
            
            \For{each collective movement $\mathcal{C}$ in collectiveMovements}{
                $rombongan.insert(\mathcal{C})$
            }
        }
    }
    \KwRet{$rombongan$}
...
Melalui proses analisis kompleksitas algoritma, algoritma identifikasi rombongan memiliki kompleksitas ruang sebesar $O(nt \mathbb{R})$ di mana $n$ merupakan jumlah entitas yang terdapat pada $\mathcal{X}$, $\mathbb{R}$ merupakan dimensi ruang gerak entitas, dan $t$ merupakan panjang interval waktu terpanjang dari setiap entitas anggota himpunan $\mathcal{X}$.

Nilai kompleksitas tersebut didapatkan dari kompleksitas tempat yang digunakan untuk menyimpan $\mathcal{X}$, di mana $\mathcal{X}$ akan menyimpan informasi posisi untuk setiap entitas pada $\mathcal{X}$ yang beranggotakan sebanyak $n$ entitas. Setiap entitas akan menyimpan posisi untuk setiap waktu sepanjang $t$. Posisi dari entitas pada satu titik waktu akan disimpan menggunakan tempat sebanyak $\mathbb{R}$ untuk setiap sumbu pada dimensi gerak entitas.
