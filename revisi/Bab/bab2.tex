...
\begin{figure}[h]
    ...
    \includegraphics[width=0.65\textwidth]{Gambar/bab2:interpolasi-lintasan.pdf}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{Gambar/bab1:sumber-data.png}
...
\begin{figure}[b]
    \centering
    \begin{subfigure}[h]{0.435\textwidth}
        \centering
...
Jarak \textit{dynamic time warping} antara dua buah lintasan dihitung dengan cara mencari seluruh \textit{warping path} dari dua lintasan tersebut. Secara formal, \textit{warping path} berdimensi $(N \times M)$ dari dua buah lintasan merupakan rangkaian $p = (p_1, p_2, \ldots, p_\ell)$ dengan $p_\ell = (x_\ell, y_\ell) \in [1 : N] \times [1 : M]\;untuk\;\ell \in [1 : L]$ yang memenuhi tiga syarat berikut:
...
\begin{enumerate}
    ...
    \item \textit{Step size}: $p_{(\ell + 1)} - p_\ell = {(1, 0), (0, 1), (1, 1)}\;untuk\; \ell \in [1 : L - 1]$.
\end{enumerate}
...
Jarak total $c_p$ dari sebuah \textit{warping path} yang valid merupakan akumulasi perbedaan jarak dari setiap pasang titik yang terdapat dalam \textit{warping path}. Secara formal, $c_p$ dapat dihitung menggunakan Rumus~\ref{bab2:warping-path-length}.
...
\begin{algorithm}[h]
    ...
    \Fn{\DTW{$X, Y$}}{
        ...
        $DTW[0, 0] \gets 0$ \\
        \For{$i \gets 1$ \KwTo{n}}{
            $DTW[i, 1] \gets d(i, 1) + DTW[i - 1, 1]$
        }
        \For{$j \gets 1$ \KwTo{m}}{
            $DTW[1, j] \gets d(1, j) + DTW[i, j - 1]$
        }
        \For{$i \gets 1$ \KwTo{n}}{
            \For{$j \gets 1$ \KwTo{m}}{
                $cost \gets d(x_i, y_j)$ \\
...
\begin{itemize}
    ...
    \begin{figure}[h]
        \centering
        \begin{subfigure}[h]{0.265\textwidth}
            \centering
        ...
        \begin{subfigure}[h]{0.265\textwidth}
            \centering
            \includegraphics[height=3.7cm]{Gambar/bab2:intermediate.pdf}
...
\begin{itemize}
    ...
    \begin{figure}[h]
        ...
        \includegraphics[width=0.75\textwidth]{Gambar/bab2:flock.pdf}
...
\begin{figure}[h]
    \centering
    \captionsetup{width=0.75\textwidth}
    \includegraphics[width=0.95\textwidth]{Gambar/bab2:group.pdf}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.65\textwidth]{Gambar/bab2:proksemik.pdf}
...
Terdapat dua cara yang dapat digunakan untuk melakukan evaluasi terhadap definisi dari pergerakan kolektif, yaitu secara kuantitatif dan kualitatif.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{Gambar/bab2:contoh-visualisasi.pdf}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.4\textwidth]{Gambar/bab2:entitas.pdf}
...
\begin{figure}[h]
    ...
    \begin{subfigure}[h]{0.3\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Gambar/bab2:contoh-entitas-1.pdf}
        \caption{Hasil identifikasi manusia = $\{4, 6, 7\}$ \\ Hasil identifikasi definisi pergerakan kolektif = $\{\}$}
    \end{subfigure} \hspace{0.25cm}
    \begin{subfigure}[h]{0.3\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Gambar/bab2:contoh-entitas-2.pdf}
        \caption{Hasil identifikasi manusia = $\{4, 6, 7\}$ \\ Hasil identifikasi definisi pergerakan kolektif = $\{4, 6\}$}
    \end{subfigure} \hspace{0.25cm}
    \begin{subfigure}[h]{0.3\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Gambar/bab2:contoh-entitas-3.pdf}
